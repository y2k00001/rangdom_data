...
from utils import password, telphone, gen_name, gen_email, gen_unique_id, gen_plate_no, gen_lucky,gen_id_num,gen_com
...
@random.route('/random/com',methods=['GET'])
def com():
    num = 1 if not request.args.get("num") else int(request.args.get("num"))
    return api_return("OK",data=gen_com(num))
